...
printf '\e]8;;http://localhost:3000\aClick Here\e]8;;\a\n'
...
license() {
    ...
    local topLevel="$(realpath $repo)"
    local current="$(realpath $pwd)"

    local target="${current}";

    if [[ $topLevel != $current ]]; then
        if [[ -n $topLevel ]]; then
            echo "Place LICENSE file in $current or $topLevel?"
            read -k 1 "select?Select:
...
[2]: ${topLevel}"
            if [[ $select == "1" ]]; then
                ...
                target="$(realpath $topLevel)";
            else
                echo -e "\033[31mError: Invalid selection: \033[205;100;123m${select}"
                exit 1
            fi
        else
            echo -e "\033[31mError: Invalid target: \033[205;100;123m${topLevel}"
            exit 1
        fi
    else
        echo -e "\033[38;2;10;240;170mgit\033[0m repository detected. Writing LICENSE file..."
	fi

        echo "Copyright (c) 2026 Donald Wayne Moore Jr. <donald@imgnx.org>
...
OR IN CONNECTION WITH THE USE OR PERFORMANCE OF THIS SOFTWARE." > $topLevel/LICENSE
...
screenshots() {
    # The -p flag in Zsh allows you to specify the prompt text
    read -k 1 "separate?Would you like to place them in a separate folder? (y/n): "
    echo # Adds a newline so the script doesn't continue on the same line as the prompt

    if [[ "$separate" == "y" || "$separate" == "Y" ]]; then
        mkdir -p "screenshots"
        cd "screenshots" # Using cd is often simpler for a quick script
    fi

    ffmpeg -i "$1" -vf fps=1 screenshot_%03d.jpg

    # If we entered a folder, we don't necessarily need popd unless
    # there are more commands following this in the original directory.
}
...
befall() {
    num="$(random)"
    echo "Now use this number ($num) to pick a random word in the dictionary."
}

chatgpt() {
    echo "Todo: Curl the API or something..."
}

frmwrk() {
    cd $MODS/taku-framework
}
...
tsrv() {
    ...
    then
        ...
        while true; do
            /Users/donaldmoore/src/dinglehopper/tsrv/status > "$HOME/.cache/pm2/status.txt" 2>&1 &
            pid=$!
            wait "$pid"
            clear
            cat "$HOME/.cache/pm2/status.txt"
            sleep 9
        done
    else
        cd /Users/donaldmoore/src/dinglehopper/tsrv
        echo -e "\033[48;2;255;205;0m\033[38;2;0;0;0m\033[1mRun \`tsrv status\` to get a live-reloading (seamless) pm2 status table.\033[0m"
    fi
}
...
_module_nav() {
	...
	print -u2 -- "module-nav: unknown root: $name"
	return 1
    fi
	...
	print -u2 -- "$resolved: no such directory: $path"
	return 1
    fi
}

for __module_nav_name in ${(k)MODULE_NAV_MODULES}; do
    eval "${__module_nav_name}() { _module_nav ${__module_nav_name} \"\$@\"; }"
done
unset __module_nav_name

for __module_nav_alias __module_nav_target in ${(kv)MODULE_NAV_ALIASES}; do
    alias "${__module_nav_alias}=${__module_nav_target}"
done
unset __module_nav_alias __module_nav_target
...
noiphone() {
    ...
    if xferdefaults read $pref SuppressDeviceDisconnectAlerts 2>/dev/null | grep -q 1; then
...
where() {
    ...
    {
	: "ID:$uuid" # Unique tag for searching

	local ONE
	typeset -A TWO

	# 1. Find breadcrumb files
	ONE=$(find . "$@" -type f -name ".breadcrumb*") && {
	    # 2. Parse file stats into associative array
	    while IFS= read -r line; do
		TWO["$line"]=$(stat -f '%m %N' "$line")
	    done <<< "$ONE"

	    # 3. Sort by modification time and display with 'bat'
	    for k in "${(@onk)TWO}"; do
		echo "${TWO[$k]}"
	    done | sort -hr |  bat
...
gtkpr() {
	...
	sudo spctl --master-disxfer	defaults write com.apple.LaunchServices LSQuarantine -bool false
...
function tree() {
    ...
    for arg in "$@"; do
        # Check if it's a flag (starts with - but isn't a double dash)
        if [[ "$arg" =~ ^-[^-]+ ]]; then
            # If it contains 'a', disable gitignore
            if [[ "$arg" == *a* ]]; then
                use_gitignore=false
            fi
            # If it contains 'p', disable color and warning
            if [[ "$arg" == *p* ]]; then
                use_color=false
                show_warning=false
            fi

            # Rebuild the flag removing 'p' if you don't want it passed to real tree
            # For simplicity, we just pass the flag through here:
    ...
    local final_args=()
    [[ "$use_color" == true ]] && final_args+=("-C")
    [[ "$use_gitignore" == true ]] && final_args+=("--gitignore")

    /opt/homebrew/bin/tree "${final_args[@]}" "${passthrough_args[@]}"

    if [[ "$show_warning" == true ]]; then
        echo -e "\n\033[38;2;255;205;0mNotice: \`tree\` is wrapped to ignore files from .gitignore and use color.\033[0m"
    fi
}

function myTree() {
    local argString="-C --gitignore"
    local outputWarning="1"

    echo "$@"

    for arg in "$@"; do
	case " $* " in
	    *"-a"*) # All files (even hidden)
		echo "\033[38;2;255;205;0mNotice: Including all directories and files (including hidden ones)."
		argString=$(echo argString | sed s/--gitignore//g)

		;;
	    *"-p"*) # Plain style
		outputWarning="0"
		argString=$(echo argString | sed s/-C//g)
		shift
		;;
	    *)
    ...
    echo "\033[38;2;255;205;0mOutput:\033[0m /opt/homebrew/bin/tree \"$argString\" \"$@\""

    /opt/homebrew/bin/tree "$argString" "$@"

    if [[ outputWarning == "1" ]]; then
